...
class ProtocolType {
 public:
  ...
  virtual Int32 SetIp(const string& ip) = 0;
  virtual void SetPort(PortType port) = 0;
  virtual sockaddr* GetSocketAddress() const = 0;
  virtual size_t AddressLength() const = 0;
  virtual string GetIp() const = 0;
  virtual PortType GetPort() const = 0;
};
...
class IPV4_TCP : public ProtocolType {
 ...
 public:
  ...
  virtual Int32 SetIp(const string& ip = "") {
    if (ip.empty()) {
      address_.sin_addr.s_addr = htonl(INADDR_ANY);
      return 0;
    } else {
  ...
  virtual void SetPort(PortType port) {
  ...
  virtual PortType GetPort() const {
...
class IPV6_TCP : public ProtocolType {
 ...
 public:
  virtual void SetSocketAddress(const SocketAddressType &socket_address) {
    memcpy(&address_, &socket_address, sizeof(socket_address));
  }
