...
TimerID TimerQueue::allot_timer_id_ = 0;
...
TimerID TimerQueue::Push(TimerEvent* timer_event) {
	assert(timer_event);
	timer_event->SetTimerId(++allot_timer_id_);
	timer_queue_.push_back(timer_event);
	push_heap(timer_queue_.begin(), timer_queue_.end(),
	          TimerEventCompare());
	return allot_timer_id_;
}

void TimerQueue::Pop() {
	pop_heap(timer_queue_.begin(), timer_queue_.end(),
			TimerEventCompare());
...
int TimerQueue::Remove(Handler* handler) {
	assert(handler);
	TimerVector::iterator iter = remove_if(timer_queue_.begin(),
				timer_queue_.end(), TimerEventHandlerEqual(handler));
	if (iter != timer_queue_.end()) {
		...
		make_heap(timer_queue_.begin(), timer_queue_.end(),
		          TimerEventCompare());
...
int TimerQueue::Remove(TimerID timer_id) {
	// TODO: 可性能优化，自己实现删除一个元素后堆的调整
	TimerVector::iterator iter = remove_if(timer_queue_.begin(),
				timer_queue_.end(), TimerEventTimerIDEqual(timer_id));
	if (iter != timer_queue_.end()) {
		int remove_number = 0;
		for (TimerVector::iterator delete_iter = iter;
				delete_iter != timer_queue_.end(); ++delete_iter) {
