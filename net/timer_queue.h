...
class TimerQueue {
 ...
 public:
	...
	// 返回 timer_id
	TimerID Push(TimerEvent* timer_event);
	...
	int Remove(TimerID timer_id);

	// 返回值为被移除个数，且将为被移除的 TimerEvent* 项执行了 delete 操作
	int Remove(Handler* handler);

 private:
	static TimerID allot_timer_id_;
	TimerVector timer_queue_;
};

class TimerEvent {
 public:
	TimerEvent(Handler* handler,
	           const TimeValue& activation_time,
...
class TimerEventTimerIDEqual {
 public:
	TimerEventTimerIDEqual(TimerID timer_id) : timer_id_(timer_id) {
		assert(timer_id > 0);
	}
